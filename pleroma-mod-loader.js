// https://gist.github.com/falsycat/7ce1e340daebb1bfc9f1e3b3b451b3c5
(() => {
  const kModPath = '/instance/';

  window.addEventListener('DOMContentLoaded', () => {
    const mods = [mod_style, mod_highlight, mod_math];
      ...
      for (const mut of muts) {
        for (const e of mut.addedNodes) {
          ...
          for (const st of Array.from(
            e.querySelectorAll('.Conversation .Status:not(.mod-altered)')
          )) {
            for (const mod of mods) {
  ...
  const mod_highlight = {
    ...
    alter: async (st) => {
      let pro = Array.from(document.querySelectorAll('pre code')).map(
        async (e) => {
    ...
    alter: async (st) => {
      ...
      paras.forEach(async (e) => {
        ...
        if (match) {
          ...
          pro.push(
            mod_math.init_lazy().then(() => {
              e.innerHTML = katex.renderToString(expr, {
                output: 'html',
                displayMode: true,
              });
            })
